...
var qute = 0;
d3.json('./js/source.json', function(thing) {
    db = document.getElementById("db");
    
    for (var x in thing) {
        db.innerHTML += '<option value=' + '"' + x + '"' + '>' + x + '</option>';
    };
    db.selectedIndex = -1;
    qute = thing;
});
...
function updateProbeList() {
    ...
    var selected_db = db.options[db.selectedIndex].value;

    probe.innerHTML = "";
    for (var x in qute[[selected_db]]) {
...
function updateClustList() {
    ...
    clust.innerHTML = "";
    for (var x in qute[[selected_db]][[selected_probe]]) {
        x = qute[[selected_db]][[selected_probe]][x];
        clust.innerHTML += '<option value=' + '"' + '../' + selected_db + '/' + 
            x + '.json"' + '>' + x + '</option>';
    };
